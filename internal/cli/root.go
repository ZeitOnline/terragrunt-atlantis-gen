...
func newGenerateCmd(version string) *cobra.Command {
	...
	f.BoolVar(&opts.FailOnParseErrors, "fail-on-parse-errors", false, "Fail when terragrunt find suppressed a parse error: a unit whose config does not parse keeps its project but watches fewer paths. Default is to report them in the log")
